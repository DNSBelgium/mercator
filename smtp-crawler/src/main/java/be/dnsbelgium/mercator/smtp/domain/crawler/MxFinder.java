...
public class MxFinder {
    ...
    public MxFinder(
            ...
            @Value("${smtp.crawler.recursive.resolver.timeOut.ms:2500}")  int timeOutMs) throws UnknownHostException {
        logger.info("Initializing MxFinder with recursiveResolver hostname={} retries={} timeOut={}ms",
                recursiveResolver, retries, timeOutMs);
        resolver = new ExtendedResolver();
        resolver.addResolver(new SimpleResolver(recursiveResolver));
    ...
    public MxLookupResult findMxRecordsFor(String domainName) {
        ...
        for (Record answer : answers) {
            if (answer instanceof MXRecord) {
                MXRecord mxRecord = (MXRecord) answer;
