...
public class WebRepository extends BaseRepository<WebCrawlResult> {
  ...
  public String getAllItemsQuery() {
    return StringSubstitutor.replace("""
      with
        crawl_result1  as (select * exclude (year, month) from read_parquet('${webCrawlDestination}/**/*.parquet', union_by_name=True)),
        html_feature1  as (select * exclude (year, month) from read_parquet('${featuresDestination}/**/*.parquet', union_by_name=True)),
        page_visit1    as (select * exclude (year, month) from read_parquet('${pageVisitDestination}/**/*.parquet', union_by_name=True)),
        crawl_result   as (select * replace(epoch(crawl_started) as crawl_started, epoch(crawl_finished) as crawl_finished)  from crawl_result1),
        html_feature   as (select * replace(epoch(crawl_timestamp) as crawl_timestamp) from html_feature1),
        page_visit     as (select * replace(epoch(crawl_started) as crawl_started, epoch(crawl_finished) as crawl_finished) from page_visit1),
  ...
  public void storeResults(String jsonResultsLocation) {
    ...
    String copyWebCrawlResult = StringSubstitutor.replace("""
            ...
            to '${webCrawlDestination}'
            (FORMAT parquet, PARTITION_BY (year, month), OVERWRITE_OR_IGNORE, FILENAME_PATTERN 'web_{uuid}' )
            """, Map.of(
