...
class DnsCrawlServiceTest {
  ...
  public void invalidDomainNameIsIgnored() {
    VisitRequest visitRequest = new VisitRequest(UUID.randomUUID(), "--invalid--.be");
  ...
  public void query_for_ascii_and_save_unicode() {
    VisitRequest visitRequest = new VisitRequest(UUID.randomUUID(), "dnsbelgië.be");
  ...
  @Test void buildEntitySuccess() {
    ...
    var request = DnsRequest.success("@", A, records);
    ZonedDateTime before = ZonedDateTime.now();
    Request entity = dnsCrawlService.buildEntity(visitRequest, request);
    ZonedDateTime after = ZonedDateTime.now();
    logger.info("entity = " + entity);
    assertThat(entity.getRecordType()).isEqualTo(A);
    assertThat(entity.getId()).isNull();
    assertThat(entity.getResponses()).hasSize(2);
    assertThat(entity.getPrefix()).isEqualTo("@");
    assertThat(entity.getNumOfResponses()).isEqualTo(2);
    assertThat(entity.getRcode()).isEqualTo(0);
    assertThat(entity.getVisitId()).isEqualTo(visitId);
    assertThat(entity.getProblem()).isNull();
    assertThat(entity.getCrawlTimestamp()).isBeforeOrEqualTo(ZonedDateTime.now());
    assertThat(entity.getCrawlTimestamp()).isBetween(before,after);
    assertThat(entity.getDomainName()).isEqualTo("dnsbelgium.be");
    assertThat(entity.isOk()).isTrue();
    assertThat(entity.getResponses().get(0).getRecordData()).isEqualTo(IP1);
    assertThat(entity.getResponses().get(1).getRecordData()).isEqualTo(IP2);
    assertThat(entity.getResponses().get(0).getTtl()).isEqualTo(TTL);
    assertThat(entity.getResponses().get(1).getTtl()).isEqualTo(TTL);
  ...
  @Test void buildEntityNxdomain() {
    UUID visitId = UUID.randomUUID();
    VisitRequest visitRequest = new VisitRequest(visitId, "dnsbelgium.be");
    var request = DnsRequest.nxdomain("@", A);
    ZonedDateTime before = ZonedDateTime.now();
    Request entity = dnsCrawlService.buildEntity(visitRequest, request);
    ZonedDateTime after = ZonedDateTime.now();
    logger.info("entity = " + entity);
    assertThat(entity.getRecordType()).isEqualTo(A);
    assertThat(entity.getId()).isNull();
    assertThat(entity.getResponses()).hasSize(0);
    assertThat(entity.getPrefix()).isEqualTo("@");
    assertThat(entity.getNumOfResponses()).isEqualTo(0);
    assertThat(entity.getRcode()).isEqualTo(Rcode.NXDOMAIN);
    assertThat(entity.getVisitId()).isEqualTo(visitId);
    assertThat(entity.getProblem()).isEqualTo("nxdomain");
    assertThat(entity.getCrawlTimestamp()).isBeforeOrEqualTo(ZonedDateTime.now());
    assertThat(entity.getCrawlTimestamp()).isBetween(before,after);
    assertThat(entity.getDomainName()).isEqualTo("dnsbelgium.be");
  ...
  private DnsRequest randomNonEmptyResponse() {
    List<RRecord> records = new ArrayList<>();
    int numResponses = RandomUtils.nextInt(1,5);
